...
CRITICAL - PRICE SCALE READING (for accurate zone placement):
- Look CAREFULLY at the RIGHT EDGE price scale labels
- Find the topmost visible price label (read exact number including all decimals)
- Find the bottommost visible price label (read exact number including all decimals)
- Example: If you see "1.16234" at top and "1.15450" at bottom, report those EXACT values
- DO NOT round or estimate - accuracy is critical for correct zone placement
...
- Find the topmost visible price label (read exact number including all decimals)
- Find the bottommost visible price label (read exact number including all decimals)
- Example: If you see "1.16298" at top and "1.15401" at bottom, report those EXACT values
- DO NOT round or estimate - accuracy is critical
...
Return ONLY valid JSON:
{
  ...
  validate15MinAnalysis(result) {
    ...
    if (result.zone_price_high && result.zone_price_low && result.pair) {
      ...
      if (!zoneValidation.valid) {
        ...
        logger.warn(`15-min zone validation warning: ${zoneValidation.error}`);
      } else {
  ...
  validate5MinAnalysis(result, primaryResult) {
    ...
    if (result.zone_price_high && result.zone_price_low && result.pair) {
      ...
      if (!zoneValidation.valid) {
        warnings.push(zoneValidation.error);
        logger.warn(`5-min zone validation warning: ${zoneValidation.error}`);
      } else {
  ...
  combineAnalysis(primaryResult, entryResult) {
    ...
    return {
      ...
      primary_zone: {
        ...
        coordinates: primaryResult.zone_coordinates || { x1: 0, y1: 0, x2: 0, y2: 0 }
      },
      entry_zone: {
        coordinates: entryResult.zone_coordinates || { x1: 0, y1: 0, x2: 0, y2: 0 },
  ...
  getDrawingInstructions(analysis) {
    ...
    return {
      primary: {
        ...
        priceHigh: analysis.primary_analysis?.zone_price_high,
        priceLow: analysis.primary_analysis?.zone_price_low,
        chartPriceHigh: analysis.primary_analysis?.chart_price_high,
        chartPriceLow: analysis.primary_analysis?.chart_price_low
      },
      entry: {
        ...
        priceHigh: analysis.entry_analysis?.zone_price_high,
        priceLow: analysis.entry_analysis?.zone_price_low,
        chartPriceHigh: analysis.entry_analysis?.chart_price_high,
        chartPriceLow: analysis.entry_analysis?.chart_price_low
      }
    };
  }
}
