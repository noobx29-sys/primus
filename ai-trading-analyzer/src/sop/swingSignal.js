...
Return ONLY valid JSON:
{
  ...
  validateDailyAnalysis(result) {
    ...
    if (result.zone_price_high && result.zone_price_low && result.pair) {
      ...
      if (!zoneValidation.valid) {
        ...
        logger.warn(`Daily zone validation warning: ${zoneValidation.error}`);
      } else {
  ...
  validateM30Analysis(result, dailyResult) {
    ...
    if (result.zone_price_high && result.zone_price_low && result.pair) {
      ...
      if (!zoneValidation.valid) {
        warnings.push(zoneValidation.error);
        logger.warn(`M30 zone validation warning: ${zoneValidation.error}`);
      } else {
  ...
  getDrawingInstructions(analysis) {
    ...
    return {
      daily: {
        ...
        priceHigh: analysis.daily_analysis?.zone_price_high,
        priceLow: analysis.daily_analysis?.zone_price_low,
        chartPriceHigh: analysis.daily_analysis?.chart_price_high,
        chartPriceLow: analysis.daily_analysis?.chart_price_low
      },
      m30: {
        ...
        priceHigh: analysis.m30_analysis?.zone_price_high,
        priceLow: analysis.m30_analysis?.zone_price_low,
        chartPriceHigh: analysis.m30_analysis?.chart_price_high,
        chartPriceLow: analysis.m30_analysis?.chart_price_low
      }
    };
  }
}
