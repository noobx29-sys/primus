...
class ZoneDrawer {
   ...
   */
  drawZone(zoneConfig) {
    const { coordinates, color, opacity, borderWidth, label, labelPosition, priceHigh, priceLow, chartPriceHigh, chartPriceLow } = zoneConfig;
    let { x1, y1, x2, y2 } = coordinates;

    logger.info(`Zone config received: priceHigh=${priceHigh}, priceLow=${priceLow}, chartPriceHigh=${chartPriceHigh}, chartPriceLow=${chartPriceLow}`);
    logger.info(`AI-provided coordinates: x1=${x1}, y1=${y1}, x2=${x2}, y2=${y2}`);
    logger.info(`Types: priceHigh=${typeof priceHigh}, priceLow=${typeof priceLow}, chartPriceHigh=${typeof chartPriceHigh}, chartPriceLow=${typeof chartPriceLow}`);
    logger.info(`Truthy check: priceHigh=${!!priceHigh}, priceLow=${!!priceLow}, chartPriceHigh=${!!chartPriceHigh}, chartPriceLow=${!!chartPriceLow}`);

    // If we have chart price range and zone prices, calculate correct Y coordinates
    // Use explicit null/undefined check instead of truthy check (0 is falsy but valid)
    if (chartPriceHigh != null && chartPriceLow != null && priceHigh != null && priceLow != null) {
      logger.info(`✓ All price data available, recalculating Y coordinates from prices...`);
      // Auto-correct inverted inputs from OCR/GPT
      let cHigh = chartPriceHigh;
      let cLow = chartPriceLow;
      let zHigh = priceHigh;
      let zLow = priceLow;

      if (cHigh < cLow) {
        logger.warn(`Swapped chart bounds detected: high(${cHigh}) < low(${cLow}). Auto-correcting.`);
        [cHigh, cLow] = [cLow, cHigh];
      }
      if (zHigh < zLow) {
        logger.warn(`Swapped zone bounds detected: high(${zHigh}) < low(${zLow}). Auto-correcting.`);
      ...
      // - Top ~60-80px: toolbar, indicators panel
      // - Bottom ~40-50px: time axis, status bar
      // - Price scale adds ~3-5% margin above/below for visual padding

      const topUIHeight = 70;      // TradingView top UI elements
      const bottomUIHeight = 45;   // TradingView bottom UI elements
      const plotHeight = H - topUIHeight - bottomUIHeight;

      // TradingView auto-scale margin (price padding)
      const priceMarginPercent = 0.03; // 3% is more typical for TradingView
      const priceRangeRaw = cHigh - cLow;
      const priceMargin = priceRangeRaw * priceMarginPercent;
      const adjustedHigh = cHigh + priceMargin;
      const adjustedLow = cLow - priceMargin;
      const priceRange = priceRangeRaw + (2 * priceMargin);

      // Calculate Y positions within plot area (inverted: top = high price)
      y1 = topUIHeight + ((adjustedHigh - zHigh) / priceRange) * plotHeight;
      y2 = topUIHeight + ((adjustedHigh - zLow) / priceRange) * plotHeight;

      logger.info(`✓ Calculated Y from prices: zone ${zLow}-${zHigh}, chart ${cLow}-${cHigh} → Y ${y1.toFixed(0)}-${y2.toFixed(0)}`);
    } else {
      logger.warn(`⚠ Missing price data for coordinate calculation, using GPT coordinates: Y ${y1}-${y2}`);
    }
